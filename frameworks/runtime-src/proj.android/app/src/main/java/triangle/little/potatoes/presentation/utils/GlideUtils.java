package triangle.little.potatoes.presentation.utils;
...
public class GlideUtils {
    ...
    public static void loadImageViewLoading(Context mContext, String path, ImageView mImageView, int loadingImage, int errorImage) {
        Glide.with(mContext).load(path).placeholder(loadingImage).error(errorImage).into(mImageView);
    }
    //设置加载中以及加载失败图片不启用加载动画
    public static void loadImageDoNoAni(Context mContext, String path, ImageView mImageView, int loadingImage, int errorImage) {
        Glide.with(mContext).load(path).placeholder(loadingImage).error(errorImage).dontAnimate().into(mImageView);
    }
    //设置加载中以及加载失败图片并且指定大小
    public static void loadImageViewLoadingSize(Context mContext, String path, int width, int height, ImageView mImageView, int loadingImage, int errorImageView) {
        Glide.with(mContext).load(path).placeholder(loadingImage).error(errorImageView).override(width, height).into(mImageView);
    }
    ...
    //根据加载的图片宽高比来动态设置图片高度
    public static void loadImageFitSize(final Context context, String url, final int width,
                                        final ImageView imageView, final int loadingImage, final int errorImage) {
        Glide.with(context).load(url).listener(new RequestListener<Drawable>() {
            ...
            public boolean onResourceReady(Drawable resource, Object model, Target<Drawable> target, DataSource dataSource, boolean isFirstResource) {
                int imageWidth = resource.getIntrinsicWidth();
                    ...
                    params.width = width;
                    int height = params.width * imageHeight / imageWidth;
            ...
            //    return false;
            //}
        }).placeholder(loadingImage).error(errorImage).into(imageView);
    }
}